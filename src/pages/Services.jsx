    ...
    <main className="bg-[#0a0e27]">
      ...
      <section className="relative py-24 px-6 dark-section overflow-hidden min-h-96 flex items-center">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute bottom-0 left-1/4 w-80 h-80 bg-[#ffcf33] rounded-full blur-3xl opacity-5 animate-float" style={{ animationDelay: '2s' }}></div>
        </div>

        <div className="max-w-5xl mx-auto text-center relative z-10">
          <h1 className="text-6xl md:text-7xl font-black mb-6 gradient-text">OUR SERVICES</h1>
          <p className="text-xl text-white/90 max-w-3xl mx-auto">
            From websites to AI integration, we deliver innovative digital solutions that drive growth and solve real business challenges.
          </p>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {SERVICES.map((service, i) => (
              ...
              >
                ...
                <h3 className="text-2xl font-black text-[#00d4ff] mb-3 uppercase">{service.title}</h3>
                <p className="text-white/80 mb-4 leading-relaxed">{service.desc}</p>
                <div className="space-y-2">
                  {service.details.map((detail, j) => (
                    <div key={j} className="flex items-center text-sm text-[#ffcf33]">
      ...
      <section className="py-24 px-6 dark-section relative overflow-hidden">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-5xl font-black text-center mb-16 gradient-text">OUR PROCESS</h2>
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { step: '01', title: 'DISCOVERY', desc: 'Understand your goals and vision' },
              { step: '02', title: 'PLANNING', desc: 'Create detailed project roadmap' },
              { step: '03', title: 'EXECUTION', desc: 'Build with precision and care' },
              { step: '04', title: 'LAUNCH', desc: 'Deploy and celebrate success' },
            ].map((item, i) => (
              <div key={i} className="neon-card p-6 rounded-xl text-center hover:scale-105 transition-transform">
                <div className="text-4xl font-black text-[#00d4ff] mb-2">{item.step}</div>
                <h3 className="text-lg font-bold text-[#ffcf33] mb-2 uppercase">{item.title}</h3>
      ...
      <section className="py-24 px-6 darker-section relative">
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <h2 className="text-5xl font-black mb-6 gradient-text">READY TO BUILD?</h2>
          <p className="text-xl text-white/90 mb-8">Choose your service and let's create something incredible.</p>
